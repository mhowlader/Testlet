...
### Abstract:
Teslet is a Quizlet inspired project which revolves around duplicating Quizlet’s core functionality: logging in, creating, saving, and viewing sets.

### How To Procure Your Own API Keys:
This project utilizes various APIs, so the user needs a json file with the API keys formatted as such (the skeleton is provided under `/keys/<api_name>`):
...
### Dependencies: 
Dependencies, as listed in requirements.txt, are as follows:
...
Install dependencies with the follow command in the root directory of our repo:
```
...
**ssh**
```
git@github.com:mhowlader/Testlet.git
```

**https**
```
https://github.com/mhowlader/Testlet.git
```
